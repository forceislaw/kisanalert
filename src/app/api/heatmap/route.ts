...
export async function GET(req: NextRequest) {
  try {
    ...
    type RawReport = { district_id: number | null; severity_level: string; created_at: string }

    const reports = (rawReports || []) as RawReport[]
    ...
    const fakeReports = generateFakeReports(RAIN_SHADOW_DISTRICTS.length, 80).filter(r => {
      const since = new Date()
      since.setDate(since.getDate() - days)
      return new Date(r.reported_at) >= since
    })
